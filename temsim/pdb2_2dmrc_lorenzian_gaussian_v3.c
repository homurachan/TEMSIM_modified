...
solv=0;//	remove solvent function
...
data=new EMData;
data->setSize(box,box);
data->zero();

if (solv) {
	rdata=new EMData;
	rdata->setSize(box,box);
	rdata->one();
...
//				d[i+j*box+k*box*box]+=kn*atoms[l].a*exp(-r*rp);
				r_in_ang=r*apix;
				if( r_in_ang < 1.0e-2 ) r_in_ang = 1.0e-2;
	...
	/*			for( II=2*3; II<2*(3+3); II+=2 ) {
		...
		//		r_in_ang=r_in_ang*r_in_ang;
				d[i+j*box]+= float(vzatomLUT(int(atoms[l].a),r_in_ang));
...
//	data->setXYZOrigin(-box/2*apix+xr[2],-box/2*apix+yr[2],-box/2*apix+zr[2]);
	data->setXYZOrigin(-box/2*apix+xr[2],-box/2*apix+yr[2],0);
}

else {
//	data->setXYZOrigin(-box/2*apix,-box/2*apix,-box/2*apix);
	data->setXYZOrigin(-box/2*apix,-box/2*apix,0);
}
