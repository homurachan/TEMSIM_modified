...
const double OWEIGHT=16;
const double ODENSITY=0.835;
...
int ReadXYZcoord_modified( const char* infile, const int ncellx, const int ncelly,
    const int ncellz, float *ax, float *by, float *cz, int** Znum, 
    float** x, float** y, float** z, float** occ, float **wobble,
    string &line1, float oxygen_radius);
void display_usage(){
	cout<<"This program produce multislice tif before imaging."<<endl;
	cout<<"-a <apix>"<<endl;
	cout<<"-y <ySize>"<<endl;
	cout<<"-i <Input xyz file>"<<endl;
	cout<<"-f -l <first to last>"<<endl;
	cout<<"-v <voltage>"<<endl;
	cout<<"-z <slice thickness or dz>"<<endl;
	cout<<"-r <whether to add random oxygen at center>"<<endl;
	cout<<"-x <central oxygen radius in Angstrom>"<<endl;
	cout<<"-o <output root name>"<<endl;
}
static const char *optString = "a:y:i:f:l:v:z:r:x:o:h?";
...
int main(int argc, char *argv[])
{
    ...
    float *param, *sparam,oxygen_radius;
	...
	string tmp,input,output_root;
	float apix,volume_size;
	int first,last;
	...
	while(opt!=-1) {
		switch(opt) {
			...
			case 'z':
				tmp=optarg;
				deltaz=double(atof(tmp.c_str()));
				break;
			case 'r':
				tmp=optarg;
				whether_round_oxygen=atoi(tmp.c_str());
				break;
			case 'x':
				tmp=optarg;
				oxygen_radius=atof(tmp.c_str());
				break;
			case 'o':
				output_root=optarg;
				break;
...
//	cout<<"If you like to add center random oxygen 0/1?"<<endl;
//	cin>>whether_round_oxygen;
//	if(whether_round_oxygen){
//		cout<<"Enter oxygen radius:"<<endl;
//		cin>>oxygen_radius;
//		cout<<"oxygen density is 1.7g per cm^3."<<endl;
//	}
	...
	if(whether_round_oxygen)
		natom = ReadXYZcoord_modified( filein0.c_str(), ncellx, ncelly, ncellz,
			&ax, &by, &cz, &Znum, &x, &y, &z, &occ, &wobble,
			description, oxygen_radius);
	else
